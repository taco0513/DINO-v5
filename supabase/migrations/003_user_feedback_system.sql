-- User Feedback System for Community-Driven Visa Intelligence
-- Migration: 003_user_feedback_system
...
CREATE TABLE country_data_accuracy (
...
COMMENT ON TABLE user_reports IS 'Community feedback on visa information accuracy';
COMMENT ON TABLE admin_notifications IS 'Notifications for administrators about user feedback';
COMMENT ON TABLE country_data_accuracy IS 'Tracking data accuracy and freshness by country';

COMMENT ON COLUMN user_reports.report_type IS 'Type of issue being reported';
COMMENT ON COLUMN user_reports.user_experience IS 'What the user actually experienced';
COMMENT ON COLUMN user_reports.current_app_data IS 'What the app currently shows';
COMMENT ON COLUMN user_reports.verification_count IS 'Number of users reporting same issue';
COMMENT ON COLUMN user_reports.confidence_score IS 'Calculated confidence based on multiple factors';